...
class OmokBoard extends Canvas{               // 오목판을 구현하는 클래스
  ...
  synchronized private void drawBoard(Graphics g){      // 오목판을 그린다.
    ...
    drawStones();
    gbuff.setColor(Color.black);
    gbuff.setFont(new Font("굴림", Font.BOLD, 15));
...
public class OmokClient extends JFrame implements Runnable, ActionListener{
	static Clip clip;
	static void PlaySound_(File Sound) {
		  try {
  ...
  static private Button startButton=new Button("대국 시작");    // 대국 시작 버튼
  ...
  // 각종 정보를 보여주는 레이블
  MainController main;
  private JLabel infoView=new JLabel("                    Omok Battle");
  ...
  private String userName=main.loginframe.textField.getText();        // 사용자 이름
  ...
  public OmokClient(String title){                        // 생성자
    ...
    infoView.setFont(new Font("굴림", Font.BOLD, 23));
    infoView.setBounds(14,34,480,30);
    infoView.setBackground(UIManager.getColor("menu"));
    board.setLocation(10,70);
    JPanel p=new JPanel();
    /*public void paintComponent(Graphics g) {
        g.drawImage(icon12.getImage(), 0, 0, null);
        setOpaque(false);
 ...
 p.add(nameBox);
  ...
  //label_1.setBackground(UIManager.getColor("menu"));
  p.add(label_1); 
  p.add(roomBox);
  p.add(enterButton); 
  p.add(exitButton);
  ...
  exitButton.addActionListener(this);
    ...
    p3.setBounds(500, 300, 250,250);getContentPane().add(p2); 
 getContentPane().add(p3);
 label.setBackground(UIManager.getColor("windowBorder"));
 label.setBounds(0, 0, 924, 626);
 
 getContentPane().add(label);
 im1 = new ImageIcon("노랑.jpg"); 
 label.setIcon(im1);
  ...
  public void run(){
    ...
    while((msg=reader.readLine())!=null){
        ...
        if(msg.startsWith("[STONE]")){     // 상대편이 놓은 돌의 좌표
          ...
          JOptionPane.showMessageDialog(null, "이겼습니다.","승리",JOptionPane.INFORMATION_MESSAGE);
          endGame("이겼습니다.");
          clip.start();
        }
 
        else if(msg.startsWith("[LOSE]")) {            // 졌으면
          JOptionPane.showMessageDialog(null, "졌습니다.","패배",JOptionPane.INFORMATION_MESSAGE);
          endGame("졌습니다.");
          clip.start();
  ...
  private void endGame(String msg){                // 게임의 종료시키는 메소드
    ...
    try{ Thread.sleep(200); }catch(Exception e){}    // 2초간 대기
